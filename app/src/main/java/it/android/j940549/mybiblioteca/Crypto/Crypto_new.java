package it.android.j940549.mybiblioteca.Crypto;
...
public class Crypto_new {
    ...
    public Crypto_new(Context myContext) {
        setAlias(SAMPLE_ALIAS);
        this.myContext = myContext;


        try {
            createKeys(myContext);
    ...
    public void createKeys(Context context) throws NoSuchProviderException,
            NoSuchAlgorithmException, InvalidAlgorithmParameterException, KeyStoreException, IOException, CertificateException {

        KeyStore keyStore = KeyStore.getInstance("AndroidKeyStore");
        keyStore.load(null);
        //String alias="myBibioteca";
    ...
    public boolean verifyData(String input, String signatureStr) throws KeyStoreException,
        ...
        // the specified algorithm.
        Signature s = Signature.getInstance("SHA256withRSA");
        Log.w(TAG, "signature..."+s.toString());

        // BEGIN_INCLUDE(verify_data)
        // Verify the data.
        s.initVerify(((KeyStore.PrivateKeyEntry) entry).getCertificate());
        s.update(data);
        boolean esito=s.verify(signature);
        Log.w(TAG, "esito decript..."+esito);
